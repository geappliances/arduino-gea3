...
struct AsyncReadSubscription {
  AsyncReadSubscription(void* context, void (*callback)(void* context, GEA3::ReadStatus status, const void* value, uint8_t valueSize), tiny_erd_client_request_id_t requestId, tiny_erd_client_t* erdClient)
  ...
  tiny_erd_client_request_id_t requestId;
  tiny_erd_client_t* erdClient;
  tiny_event_subscription_t subscription;
};

struct AsyncWriteSubscription {
  AsyncWriteSubscription(void* context, void (*callback)(void* context, GEA3::WriteStatus status), tiny_erd_client_request_id_t requestId, tiny_erd_client_t* erdClient)
    : context(context), callback(callback), requestId(requestId), erdClient(erdClient), subscription()
  {
  }
...
GEA3::PacketListener GEA3::onPacketReceived(void* context, void (*callback)(void* context, const GEA3::Packet& packet))
{
  ...
  tiny_event_subscription_init(
    &subscription->subscription, subscription, +[](void* context, const void* args_) {
      ...
      auto args = reinterpret_cast<const tiny_gea3_interface_on_receive_args_t*>(args_);
      auto packet = Packet(args->packet);
      subscription->callback(subscription->context, packet);
    });
  tiny_event_subscribe(tiny_gea3_interface_on_receive(&gea3Interface.interface), &subscription->subscription);
...
{
  tiny_erd_client_request_id_t requestId;
  tiny_erd_client_read(&erdClient.interface, &requestId, address, erd);
  ...
  tiny_event_subscription_init(
    &subscription->subscription, subscription, +[](void* context, const void* args_) {
      ...
      auto args = reinterpret_cast<const tiny_erd_client_on_activity_args_t*>(args_);

      if((args->type == tiny_erd_client_activity_type_read_completed) && (args->read_completed.request_id == subscription->requestId)) {
        subscription->callback(subscription->context, ReadStatus::success, args->read_completed.data, args->read_completed.data_size);
        tiny_event_unsubscribe(tiny_erd_client_on_activity(&subscription->erdClient->interface), &subscription->subscription);
        delete subscription;
      }
      else if((args->type == tiny_erd_client_activity_type_read_failed) && (args->read_failed.request_id == subscription->requestId)) {
        switch(args->read_failed.reason) {
          case tiny_erd_client_read_failure_reason_retries_exhausted:
            subscription->callback(subscription->context, ReadStatus::retriesExhausted, nullptr, 0);
            break;

          default:
          case tiny_erd_client_read_failure_reason_not_supported:
  ...
  tiny_event_subscribe(tiny_erd_client_on_activity(&erdClient.interface), &subscription->subscription);
}
...
{
  tiny_erd_client_request_id_t requestId;
  tiny_erd_client_write(&erdClient.interface, &requestId, address, erd, value, valueSize);
  ...
  tiny_event_subscription_init(
    &subscription->subscription, subscription, +[](void* context, const void* args_) {
      ...
      if((args->type == tiny_erd_client_activity_type_write_completed) && (args->write_completed.request_id == subscription->requestId)) {
        subscription->callback(subscription->context, WriteStatus::success);
        tiny_event_unsubscribe(tiny_erd_client_on_activity(&subscription->erdClient->interface), &subscription->subscription);
        delete subscription;
      }
      else if((args->type == tiny_erd_client_activity_type_write_failed) && (args->write_failed.request_id == subscription->requestId)) {
...
GEA3::ErdSubscription GEA3::subscribe(uint8_t address, void* context, void (*callback)(void* context, uint16_t erd, const void* value, uint8_t valueSize))
{
  ...
  tiny_event_subscription_init(
    &subscription->subscription, subscription, +[](void* context, const void* args_) {
      ...
      if(args->address == subscription->address) {
        if(args->type == tiny_erd_client_activity_type_subscription_publication_received) {
        ...
        else if(args->type == tiny_erd_client_activity_type_subscription_host_came_online) {
          tiny_erd_client_retain_subscription(subscription->erdClient, subscription->address);
        }
      }
    });
  tiny_event_subscribe(tiny_erd_client_on_activity(&erdClient.interface), &subscription->subscription);
  ...
  tiny_erd_client_subscribe(&erdClient.interface, address);
